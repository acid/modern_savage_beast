class Forums::PostsController < ApplicationController
  prepend_before_filter :find_post,      :except => [:index, :create, :monitored, :search]
  ...
  def show
    respond_to do |format|
      format.html { redirect_to forums_forum_topic_path(@post.forum_id, @post.topic_id) }
  ...
  def create
    ...
    if @topic.locked?
      respond_to do |format|
        format.html do
          ...
          redirect_to(forums_forum_topic_path(:forum_id => params[:forum_id], :id => params[:topic_id]))
        end
        format.xml do
    ...
    respond_to do |format|
      format.html do
        redirect_to forums_forum_topic_path(:forum_id => params[:forum_id], :id => params[:topic_id], :anchor => @post.dom_id, :page => params[:page] || '1')
      end
    ...
    respond_to do |format|
      format.html do
        redirect_to forums_forum_topic_path(:forum_id => params[:forum_id], :id => params[:topic_id], :anchor => 'reply-form', :page => params[:page] || '1')
      end
    ...
    respond_to do |format|
      format.html do
        redirect_to(@post.topic.frozen? ? 
          forums_forum_path(params[:forum_id]) :
          forums_forum_topic_path(:forum_id => params[:forum_id], :id => params[:topic_id], :page => params[:page]))
      end
      format.xml { head 200 }
    end
  end
